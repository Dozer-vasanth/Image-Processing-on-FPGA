set_property IOSTANDARD LVCMOS33 [get_ports {mode[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {mode[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[11]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[10]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[9]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[8]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports clk]
set_property IOSTANDARD LVCMOS33 [get_ports en_mode_led]
set_property IOSTANDARD LVCMOS33 [get_ports h_sync]
set_property IOSTANDARD LVCMOS33 [get_ports reset]
set_property IOSTANDARD LVCMOS33 [get_ports v_sync]
set_property PACKAGE_PIN E3 [get_ports clk]
set_property PACKAGE_PIN K15 [get_ports en_mode_led]
set_property PACKAGE_PIN B11 [get_ports h_sync]
set_property PACKAGE_PIN B12 [get_ports v_sync]
set_property PACKAGE_PIN J15 [get_ports reset]
set_property PACKAGE_PIN L16 [get_ports {mode[0]}]
set_property PACKAGE_PIN M13 [get_ports {mode[1]}]
set_property PACKAGE_PIN A3 [get_ports {pixel[0]}]
set_property PACKAGE_PIN B4 [get_ports {pixel[1]}]
set_property PACKAGE_PIN C5 [get_ports {pixel[2]}]
set_property PACKAGE_PIN A4 [get_ports {pixel[3]}]
set_property PACKAGE_PIN C6 [get_ports {pixel[4]}]
set_property PACKAGE_PIN A5 [get_ports {pixel[5]}]
set_property PACKAGE_PIN B6 [get_ports {pixel[6]}]
set_property PACKAGE_PIN A6 [get_ports {pixel[7]}]
set_property PACKAGE_PIN B7 [get_ports {pixel[8]}]
set_property PACKAGE_PIN C7 [get_ports {pixel[9]}]
set_property PACKAGE_PIN D7 [get_ports {pixel[10]}]
set_property PACKAGE_PIN D8 [get_ports {pixel[11]}]
